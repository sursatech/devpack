...
func TestRust_Dev_UsesCargoRun(t *testing.T) {
	...
	// In development mode, should use cargo run
	require.Equal(t, "cargo run", ctx.Deploy.StartCmd)
	
	// Should have StartCmdHost for development mode
	require.Equal(t, "cargo run", ctx.Deploy.StartCmdHost)
...
func TestRust_GetDevStartCommand(t *testing.T) {
	...
	require.Equal(t, "cargo run", devCmd)
}
